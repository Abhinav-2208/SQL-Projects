...
INSERT INTO Stations (StationID, StationName, Location, Frequency, LaunchDate) VALUES
...
INSERT INTO Hosts (HostID, HostName, ShowCount, JoinDate) VALUES
...
INSERT INTO Shows (ShowID, ShowName, HostID, StationID, LaunchDate) VALUES
...
INSERT INTO Partnerships (PartnershipID, PartnerName, PartnershipType, StartDate, EndDate) VALUES
...
INSERT INTO ShowPartnerships (ShowID, PartnershipID) VALUES
...
INSERT INTO Awards (AwardID, AwardName, Year, ShowID) VALUES
...
INSERT INTO OnlinePresence (PresenceID, PlatformName, Link, LaunchDate) VALUES
...
-- Q.1  Retrieve the names of all radio stations in the database. 
SELECT StationName FROM Stations;
-- Q.2. Find the name and join date of the host with ID3. 
SELECT HostName,JoinDate FROM Hosts WHERE HostID=3;
-- Q.3. Retrieve the show count for the show named ‘Suhaana Safar with Annu Kapoor'. 
SELECT * FROM Hosts;
SELECT * FROM Shows;
SELECT COUNT(*) FROM Hosts AS H 
JOIN Shows AS S ON H.HostID=S.HostID WHERE S.ShowName='Suhaana Safar with Annu Kapoor';
-- Q.4 List the names of all partnerships that are of type 'E-Commerce'
SELECT PartnerName
FROM Partnerships
WHERE PartnershipType = 'E-commerce';
-- Q.5. Retrieve the names of shows that won the ‘IRF Gold’ Award in 2019.
SELECT * FROM Shows;
SELECT * FROM Awards;
SELECT S.ShowName FROM Shows AS S JOIN Awards AS A ON 
S.ShowID=A.ShowID WHERE A.AwardName='IRF Gold' ;
-- Q.6. Retrieve the platform names and links for online presences launched before 2022.
SELECT PlatformName,Link FROM OnlinePresence WHERE LaunchDate<'2022-01-01';
-- Q.7. Find the frequency and the number of awards won by each show.
SELECT s.ShowID, s.ShowName, st.Frequency, COUNT(a.AwardID) AS NumberOfAwards
FROM Shows s JOIN Stations st ON s.StationID = st.StationID
LEFT JOIN Awards a ON s.ShowID = a.ShowID
GROUP BY s.ShowID, s.ShowName, st.Frequency;
SELECT * FROM Shows;
SELECT * FROM Stations;
SELECT S.ShowID,S.ShowName,SS.Frequency,COUNT(A.AwardID) AS NumberOfAwards FROM Shows AS S
JOIN Stations AS SS ON S.StationID=SS.StationID 
JOIN Awards AS A ON A.showID=S.ShowID GROUP BY S.ShowID,S.ShowName,SS.Frequency;
-- Q.8. List Hosts who have hosted more than one show, along with the names of their shows.
SELECT * FROM Hosts;
SELECT * FROM Shows;
SELECT HostName,ShowName,ShowCount FROM  Hosts AS H JOIN Shows AS S 
ON H.HostID=S.HostID WHERE ShowCount>1;

-- Q.9. Analyze the trend of online presence launch dates and provide insights into the growth or changes.
SELECT * FROM OnlinePresence;
SELECT PlatformName, MIN(LaunchDate) AS EarliestLaunchDate,
MAX(LaunchDate) AS LatestLaunchDate, COUNT(*) AS TotalPresencesLaunched
FROM OnlinePresence GROUP BY PlatformName
ORDER BY EarliestLaunchDate;
-- Q.10 Identify the top3 hosts with the highest show counts
SELECT HostID,HostName,ShowCount FROM Hosts ORDER BY ShowCount desc LIMIT 3;
